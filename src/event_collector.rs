...
use ethers::types::{H160, H256, Filter, BlockNumber, Address, Log, U64, Block, BlockId};
  ...
  for log in logs {
    ...
    if let Some(block_number) = log.block_number {
      if let Ok(Some(block)) = provider.get_block(block_number).await {
        println!("Event Type: {}  Block Number: {}", detected_event, block_number);
