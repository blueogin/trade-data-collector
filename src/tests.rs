...
use event_collector::{OrderEvent, collect_order_events, write_to_csv};
use dotenv::dotenv;
use tokio;
use mockito::Server;
...
fn test_get_contract_creation_block_success() {
  ...
  let mock_endpoint = server.mock("GET", &*url)
    .with_status(200)
...
async fn test_collect_order_events() {
  ...
  let result = collect_order_events(ws_rpc_url, contract_address, from_block, to_block, chunk_size, event_type).await;
...
fn fuzz_test() {
  ...
  runner.run(&strategy, |(from_block, to_block, event_type)| {
    let ws_rpc_url = "wss://mainnet.infura.io/ws/v3/afee43fb439a4e1794d9acad3e4a95b8";
    let contract_address = "0x0ea6d458488d1cf51695e1d6e4744e6fb715d37c";
    let chunk_size = 1_000_000;
...
fn test_write_to_csv() -> Result<(), Box<dyn Error>> {
  ...
  let events = vec![
    OrderEvent {
      ...
      txn_hash: H256::from_slice(&decode("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")?.as_slice()),
